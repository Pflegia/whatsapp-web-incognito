...
function initialize() {
  ...
  browser.runtime.sendMessage({ name: 'getOptions' }, function (options) {
    document.dispatchEvent(
      new CustomEvent('onOptionsUpdate', {
  ...
  if (firstMenuItem != undefined) {
    ...
    browser.runtime.sendMessage({ name: 'getOptions' }, function (options) {
      ...
      drop.close = function () {
        ...
        setTimeout(function () {
          originalCloseFunction.apply(drop, arguments);
        }, 100);
      };
      drop.on('open', function () {
        if (!checkInterception()) return;
        var pressedMenuItemClass =
          UIClassNames.MENU_ITEM_CLASS + ' ' + UIClassNames.MENU_ITEM_HIGHLIGHTED_CLASS + ' active menu-item-incognito';
      ...
      drop.on('close', function () {
        document
          .getElementsByClassName('menu-item-incognito')[0]
          .setAttribute('class', UIClassNames.MENU_ITEM_CLASS + ' menu-item-incognito');
...
function generateDropContent(options) {
  ...
  var readConfirmationCheckbox = options.readConfirmationsHook
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : 'unchecked ' + "'> <div class='checkmark incognito-mark" + "'> </div>";
  var onlineUpdatesCheckbox = options.onlineUpdatesHook
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : 'unchecked ' + "'> <div class='checkmark incognito-mark" + "'> </div>";
  var typingUpdatesCheckbox = options.typingUpdatesHook
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : 'unchecked ' + "'> <div class='checkmark incognito-mark" + "'> </div>";
  var saveDeletedMessagesCheckbox = options.saveDeletedMsgs
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : 'unchecked ' + "'> <div class='checkmark incognito-mark" + "'> </div>";
  var showDeviceTypeCheckbox = options.showDeviceTypes
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : 'unchecked ' + "'> <div class='checkmark incognito-mark" + "'> </div>";
  var autoReceiptCheckbox = options.autoReceiptOnReplay
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : 'unchecked ' + "'> <div class='checkmark incognito-mark" + "'> </div>";
  var allowStatusDownloadCheckbox = options.allowStatusDownload
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : 'unchecked ' + "'> <div class='checkmark incognito-mark" + "'> </div>";
    ...
    if (options.readConfirmationsHook) {
      if (options.showReadWarning) {
        Swal.fire({
          ...
          text:
            data.formattedName +
            ' will be able to tell you read the last ' +
          ...
          if (result.isConfirmed) {
            ...
            document.dispatchEvent(
              new CustomEvent('onOptionsUpdate', { detail: JSON.stringify({ showReadWarning: shouldShowReadWarning }) }),
            );
          }
        });
      } else {
...
function getTheme() {
  ...
  if (localStorage['theme'] != 'null' && localStorage['theme'] != undefined) return localStorage['theme'];
  else {
...
function getTransformXOfView(view) {
  if (view.style.transform.includes('translate')) return parseInt(view.style.transform.match(/-?[\d\.]+/g)[0]);
  else return 0;
}
...
function markMessageNodeDeviceIfPossible(messageNode, msgID) {
  ...
  document.addEventListener('onDeviceTypeReceived', async function (e) {
    ...
    if (possibleNodes.length > 0) messageNode = possibleNodes[0].childNodes[0];

    if (messageNode.getElementsByClassName('device-type-image').length > 0) return;

    var imageURL = '';
    if (deviceType == 'computer') imageURL = chrome.runtime.getURL('images/computer.svg');
    else if (deviceType == 'phone') imageURL = chrome.runtime.getURL('images/phone.svg');
    else return;
